---
title: "Tutorial 2 - Thompson Sampling"
...
In this problem set, we will be covering Thompson Sampling methods to solve multi-armed bandit problems. 


# Dataset

In addition to the Yahoo dataset from the previous tutorial, we will work with a dataset from ZOZO. ZOZO is the largest Japanese fashion e-commerce company. The company uses multi-armed bandit algorithms to recommend fashion items to users in a large-scale fashion e-commerce platform. 
...
# this version contains 10 arms
dfZozo_10 <- read.csv('../../data/clean/zozo_data/zozo_noContext_10items.csv')%>%
...
In the dataframe _df_TS_zozo_ we have gathered the results of the TS policy for $c=0.1$. This dataframe contains the following columns: 
\begin{itemize}
...
# simulate the reuslts for 10, 20, 40 arms
...
The following part of the tutorial is not mandatory, and seeks to provide extra insight into some of the challenges related to multi-armed bandits for those that are interested. This week's challenge is aggregation; what is the unit of analysis per bandit? so far, we have  always worked with 1 multi-armed bandit that serves a set of customers with 10 arms. But this is not the only form of aggregation: are many different specifications of the multi-armed bandit problem. 

\begin{itemize}
\item For instance, on a website you might have multiple spots to show products. One might decide to have a bandit per spot on the website. In the case of the ZOZO dataset, there are 3 spots on the page to show products. This increases the number of bandits from 1 to 3. 
\item One could also have a bandit per customer. Imagine we have 10 users that very frequently visit our website. In this case, the number of bandits goes from 1 to 10. 
\item We might have a bandit per spot per customer. In the case of the ZOZO dataset, if we take the 10 most frequent visitors, this would mean having $3 \cdot 10 = 30$ bandits.
\item There are a myriad of ways one can aggregate; for instance, per type of product on the ZOZO website (shirts, shoes, etc. ) or broader categories (Male clothing, Female clothing, etc.)


\end{itemize}

\textbf{Challenge}: compare current results (expected reward or regret) against results using a different level of aggregation based on the heterogeneity of user features. For example, you might cluster the users with the features, and have a different bandit per cluster. 
