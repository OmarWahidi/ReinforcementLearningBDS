...
feature_matrix <- data.frame(feature_0_dummy, feature_1_dummy, feature_2_dummy, feature_3_dummy)

PCA_features <- classicPCA(feature_matrix, 9)
set.seed(123)

fviz_nbclust(PCA_features[1:10000,], kmeans)

km.res <- kmeans(PCA_features, 2, nstart = 25)
print(km.res)

data_cluster1 <- df[which(km.res$cluster == 1), ]
data_cluster2 <- df[which(km.res$cluster == 2), ]
#data_cluster3 <- df[which(km.res$cluster == 3), ]

bandit_OfflineReplay_item <- OfflineReplayEvaluatorBandit$new(formula = reward ~ arm_item, data = df, randomize = FALSE)
bandit_OfflineReplay_item_clust1 <- OfflineReplayEvaluatorBandit$new(formula = reward ~ arm_item, data = data_cluster1, randomize = FALSE)
bandit_OfflineReplay_item_clust2 <- OfflineReplayEvaluatorBandit$new(formula = reward ~ arm_item, data = data_cluster2, randomize = FALSE)
#bandit_OfflineReplay_item_clust3 <- OfflineReplayEvaluatorBandit$new(formula = reward ~ arm_item, data = data_cluster3, randomize = FALSE)

#UCB
alpha = 0.1
UCBitem <- LinUCBDisjointPolicy$new(alpha=alpha)
UCBitem_clust1 <- LinUCBDisjointPolicy$new(alpha=alpha)
UCBitem_clust2 <- LinUCBDisjointPolicy$new(alpha=alpha)
#UCBitem_clust3 <- LinUCBDisjointPolicy$new(alpha=alpha)

agent_item <- Agent$new(UCBitem, bandit_OfflineReplay_item, name='itemUCB')
agent_item_clust1 <- Agent$new(UCBitem_clust1, bandit_OfflineReplay_item_clust1, name='itemUCB_clust1')
agent_item_clust2 <- Agent$new(UCBitem_clust2, bandit_OfflineReplay_item_clust2, name='itemUCB_clust2')
#agent_item_clust3 <- Agent$new(UCBitem_clust3, bandit_OfflineReplay_item_clust3, name='itemUCB_clust3')

size_sim=100000
n_sim=7

simulator <- Simulator$new(list(agent_item, agent_item_clust1, agent_item_clust2), # set our agents
...
print(num)
show_results_multipleagents(res,1124)
...
bandit_OfflineReplay_item_clust2 <- OfflineReplayEvaluatorBandit$new(formula = reward ~ arm_item, data = data_cluster2, randomize = FALSE)
va = 0.01
TSitem <- ContextualLinTSPolicy$new(v=va)
TSitem_clust1 <- ContextualLinTSPolicy$new(v=va)
TSitem_clust2 <- ContextualLinTSPolicy$new(v=va)
#TSitem_clust3 <- TSisjointPolicy$new(alpha=alpha)

agent_item <- Agent$new(TSitem, bandit_OfflineReplay_item, name='itemTS')
agent_item_clust1 <- Agent$new(TSitem_clust1, bandit_OfflineReplay_item_clust1, name='itemTS_clust1')
agent_item_clust2 <- Agent$new(TSitem_clust2, bandit_OfflineReplay_item_clust2, name='itemTS_clust2')
#agent_item_clust3 <- Agent$new(TSitem_clust3, bandit_OfflineReplay_item_clust3, name='itemTS_clust3')
...
num <- get_maxObsSims(res)
print(num)
show_results_multipleagents(res,1284)
